...
| native benchmarks | `crates/qvp-core/examples/bench.rs` | `docs/MEASURED.md` cites the exact command |
| browser rendering benchmark | `web-benchmark/` | self-contained harness, selected decoded fixtures, and measured results |
...
| source data | `pages/` and `index/` at the root (gitignored) | the `quran-svg-elements` release, fetched by `scripts/sync-test-data.sh`. `docs/DATA.md` records its version |
...
`.mailmap`, `.terminology.json`, and the directories in the map, including `web-benchmark/`.
