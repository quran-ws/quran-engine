...
└── example/                        SwiftUI app (`xcodegen generate` → Demo.xcodeproj, not committed)
...
`example/sync-pages.sh` fetches the `quran-engine-pages-hafs-kfgqpc.tar.gz` asset of the
...
page.maskTransition(200)                  // hide / reveal fade on the engine clock (ms; unmask() resets it)
...
`onDoubleTap` when set). Wrap it for SwiftUI with a `UIViewRepresentable` (see `example/Sources/ContentView.swift`).
...
# data: example/sync-pages.sh downloads the release bundle on the first build
...
Or run `xcodegen generate` in `example/` and open the `Demo.xcodeproj` it writes (it is not committed).
...
demo's pages, plus `docs/API.md` and `qvp.h`. Open `example/Demo.xcodeproj`, set a signing team, run — no Rust,
no data pipeline.
