...
/* table: n_paths × 8 uint32: op_start, op_count, pt_start, pt_count, flags, word, line, reserved.
...
/* Layout: all lengths in viewport px. line_spacing multiplies the printed pitch; fill_height spreads the
   nominal grid (15 lines) over viewport_h - pads. Result: vx = ox + x*scale ; vy = oy + (y + dy[line])*scale */
typedef struct { float viewport_w, viewport_h, pad_top, pad_bottom, pad_left, pad_right, line_spacing; uint32_t fill_height, nominal_lines; } QvpLayoutSpec;
typedef struct { float scale, ox, oy, content_h, pitch; uint32_t n_lines; const float* lines; /* n_lines × {dy, slot_top, slot_bottom} */ } QvpLayout;
...
void     qvp_layout(QvpPage* page, const QvpLayoutSpec* spec, QvpLayout* out);   /* out.lines valid until next qvp_layout */
int      qvp_hit_test_view(const QvpPage* page, float vx, float vy, QvpHit* out); /* viewport px via current layout */
