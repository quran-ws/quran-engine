...
enum { QVP_CATEGORY_NONE = 0, QVP_CATEGORY_HARAKA, QVP_CATEGORY_TANWEEN, QVP_CATEGORY_LETTER_DOT, QVP_CATEGORY_ORTHOGRAPHIC, QVP_CATEGORY_DABT, QVP_CATEGORY_WAQF, QVP_CATEGORY_READING_SIGN, QVP_CATEGORY_STANDALONE };
enum { QVP_DECO_AYAH_MARKER = 0, QVP_DECO_SURAH_NAME, QVP_DECO_BASMALAH, QVP_DECO_HIZB_MARK, QVP_DECO_SAJDAH_MARK };
enum { QVP_FORM_UTHMANI = 0, QVP_FORM_IMLAEI, QVP_FORM_QPC, QVP_FORM_RASM, QVP_FORM_SEARCH };
...
enum { QVP_DIV_JUZ = 0, QVP_DIV_HIZB, QVP_DIV_NISF, QVP_DIV_RUB };
/* mark ids: qvp_mark_name / qvp_mark_from_name; 0 none, 1 fatha, 2 kasra, 3 damma, 4 fathatan, 5 kasratan, 6 dammatan,
   7 shadda, 8 sukun, 9 maddah, 10 hamza, 11 wasla, 12 small-alef, 13 small-waw, 14 small-ya, 15 small-noon, 16 dot,
   17 two-dots, 18 three-dots, 19 sifr-mustadir, 20 sifr-mustatil, 21 waqf-jaiz, 22 waqf-awla, 23 wasl-awla,
   24 waqf-lazim, 25 muanaqah, 26 saktah, 27 meem-iqlab, 28 hizb, 29 sajdah, 30 sajdah-sign, 31 sajdah-line,
   32 seen-reading, 33 tashil, 34 ishmam, 35 imalah, 255 unknown */
...
typedef struct { uint16_t sura, ayah, word, line_no; uint32_t ayah_idx, line_idx; float x0, y0, x1, y1; QvpStr text; uint32_t first_path, n_paths; } QvpWordInfo;
typedef struct { uint16_t sura, ayah; uint8_t part, parts, flags, _pad; uint16_t rub; uint32_t first_word, n_words, marker_deco; float x0, y0, x1, y1; } QvpAyahInfo;
...
typedef struct { uint8_t kind, _pad; uint16_t sura, ayah, _pad2; uint32_t line; float x0, y0, x1, y1; QvpStr text; uint32_t first_path, n_paths; } QvpDecoInfo;
...
typedef struct { uint32_t ink, diacritics, dots, waqf, sifr, marker, numeral, headers, transition_ms; const uint32_t* marks /* (mark,colour) pairs */; uint32_t n_marks; } QvpTheme;
...
typedef struct { uint8_t kind /* QVP_DIV_* */, line; uint16_t n, sura, ayah; uint32_t ayah_idx; } QvpDivision;
typedef struct { uint32_t deco; uint16_t sura, ayah; uint32_t line; float cx, cy, r; uint32_t ornament_path, numeral_path; } QvpMarker;
typedef struct { uint32_t deco; uint16_t sura, ayah, juz, hizb, nisf, rub, rub_in_hizb, _pad; } QvpRosette;
typedef struct { uint32_t deco; uint16_t sura, ayah; uint32_t sign_path; } QvpSajdah;
typedef struct { uint32_t word, index, loose; } QvpMatch;
typedef struct { float x0, y0, x1, y1; uint32_t n_words, marker_deco; } QvpCropBox;
typedef struct { uint16_t n, first_page, ayah_count; uint8_t place, _pad; QvpStr arabic, latin, english; } QvpAtlasSurah;
typedef struct { uint16_t rub, sura, ayah, page; } QvpAtlasRub;
...
int32_t  qvp_find_word(const QvpPage*, uint16_t sura, uint16_t ayah, uint16_t word);   /* -1 = not on page */
...
uint32_t qvp_markers(const QvpPage*, QvpMarker* out, uint32_t cap);       /* real ayah medallions */
...
uint32_t qvp_ayah_keys(const QvpPage*, uint32_t* out, uint32_t cap);      /* sura<<16 | ayah, reading order */
uint32_t qvp_ayah_word_count(const QvpPage*, uint16_t sura, uint16_t ayah, uint32_t* complete);
int32_t  qvp_recite_map(const QvpPage*, uint16_t sura, uint16_t ayah, uint32_t n_segments, uint32_t* out, uint32_t cap); /* -1 = mismatch */
...
int32_t  qvp_attach_words(QvpPage*, const uint8_t* json, uint32_t len);   /* sidecar {"s:a:w": {"imlaei","qpc","rasm","search"}} → words updated, -1 = bad JSON */
...
uint32_t qvp_reveal_start(QvpPage*, uint32_t lit, uint32_t by_ayah, uint32_t grey, uint32_t ink, uint32_t markers, uint32_t transition_ms); /* → steps */
...
/* crop ---------------------------------------------------------------------------------- */
int      qvp_crop_box(const QvpPage*, const QvpTarget*, float pad, uint32_t keep_markers, QvpCropBox* out);
int      qvp_crop_svg(QvpPage*, const QvpTarget*, float pad, uint32_t keep_markers, uint32_t background, QvpStr* out);
...
int32_t   qvp_atlas_page_of(const QvpAtlas*, uint16_t sura, uint16_t ayah);
...
int       qvp_atlas_division(const QvpAtlas*, uint8_t kind, uint16_t n, QvpAtlasRub* out);
int32_t   qvp_atlas_division_at(const QvpAtlas*, uint8_t kind, uint16_t sura, uint16_t ayah);
