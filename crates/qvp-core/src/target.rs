...
pub enum Target {
    ...
    Word(u32),
    Words(Vec<u32>),
    /// sura:ayah — all fragments on this page
    Ayah(u16, u16),
    /// inclusive range of ayahs (sura, from, to) on this page
...
impl Page {
    pub fn resolve(&self, t: &Target) -> Vec<u32> {
        ...
        let mut v: Vec<u32> = match t {
            ...
            Target::Ayah(s, a) => (0..n).filter(|&i| d.words[i as usize].sura == *s && d.words[i as usize].ayah == *a).collect(),
            Target::AyahRange(s, a, b) => (0..n).filter(|&i| { let w = &d.words[i as usize]; w.sura == *s && w.ayah >= *a && w.ayah <= *b }).collect(),
            Target::Line(l) => d.lines.iter().filter(|x| x.line_no == *l).flat_map(|x| x.first_word as u32..(x.first_word + x.n_words) as u32).collect(),
            Target::Surah(s) => (0..n).filter(|&i| d.words[i as usize].sura == *s).collect(),
    ...
    pub fn ayah_keys(&self) -> Vec<(u16, u16)> {
        ...
        for a in &self.data().ayahs {
            if v.last() != Some(&(a.sura, a.ayah)) && !v.contains(&(a.sura, a.ayah)) {
                v.push((a.sura, a.ayah));
            }
        }
        v
    }

    /// Number of words of (sura, ayah) on this page, and whether the whole ayah is here.
    pub fn ayah_word_count(&self, sura: u16, ayah: u16) -> (u32, bool) {
        let d = self.data();
        let frags: Vec<_> = d.ayahs.iter().filter(|a| a.sura == sura && a.ayah == ayah).collect();
        let count: u32 = frags.iter().map(|a| a.n_words as u32).sum();
        let complete = frags.first().map(|a| frags.len() as u8 == a.parts).unwrap_or(false);
        (count, complete)
    }
    ...
    pub fn recite_map(&self, sura: u16, ayah: u16, n_segments: u32) -> Option<Vec<u32>> {
        let (count, complete) = self.ayah_word_count(sura, ayah);
        if !complete || count != n_segments {
            return None;
        }
        Some(self.resolve(&Target::Ayah(sura, ayah)))
    }
