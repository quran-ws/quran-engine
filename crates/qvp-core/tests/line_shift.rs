...
//! mark must carry the line of the ayah it closes, and a line's reference centre must
//! come from its body ink alone. A sajdah sign is taller than the text and used to pull
//! the centre, which snapped the ayah mark to the neighbouring line on five pages.
//! Needs dist/pages from scripts/sync-test-data.sh. Skipped when the data is missing
//! unless QVP_REQUIRE_DATA is set.
use qvp_core::Page;
use qvp_format::{IBox, PathKind};
